(function() {
    ...
    // функция получения значения cookies по имени cookie
    function getCookie(cookieName){
        var name = cookieName + "=";
        var cookieArray = document.cookie.split(';'); //получение,разбиение cookies по ; и запись в массив
    ...
    // функция получения значений параметров URL адреса
    var getUrlParameter = function(sParam) {
        var sPageURL = window.location.search.substring(1); //получение url страницы после символа ?
        var sURLVariables = sPageURL.split('&'); // формирование массива параметров url из строки с url
    ...
    var setSovcomCookie = function() {
        ...
        //если пользователь пришел на страницу напрямую(не через ссылку)
    ...
    try {
        if (null === getCookie("__sovcom_sess") || getUrlParameter('gclid') !== "(not set)" 
            || getUrlParameter('utm_source') !== "(not set)") { 
            setSovcomCookie(); 
        }
        else { 
            setCookie("__sovcom_sess", 1, 30 * 60, path, domain); 
